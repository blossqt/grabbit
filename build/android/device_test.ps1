...
# /data/local/tmp. So each file is pushed as the shell user and poured into the
# sandbox through a pipe: cat reads it, run-as writes it.
#
# First a short video to read frames from, made here: the phone has FFmpeg,
# but from run-as nothing on the network to feed it.
$python = "$env:LOCALAPPDATA\GrabbitBuild\venv\Scripts\python.exe"
$frames = Join-Path $env:TEMP 'grabbit-frame-test'
& $python (Join-Path $PSScriptRoot '..\frames_check.py') --make-video $frames | Out-Null
